<template>
  <div v-loading="loading" class="day">
    <el-row :gutter="20">
      <el-col :span="12">
        <div class="grid-content bg-purple">
          <div class="title">日销售统计</div>
          <div class="body">
            <div class="stats">
              <span class="num">{{ dayOrderCount }}</span>
              <p class="text">当日销售量（个）</p>
            </div>
            <div class="stats">
              <span class="num">{{ dayOrderAmount }}</span>
              <p class="text">当日销售额（元）</p>
            </div>
            <div class="stats">
              <span class="num">{{ dayTotalBill }}</span>
              <p class="text">当日分成（元）</p>
            </div>
          </div>
        </div>
      </el-col>
      <el-col :span="12">
        <div class="grid-content bg-purple-light">
          <div class="title">月销售统计</div>
          <div class="body">
            <div class="stats">
              <span class="num monthNum">{{ monthOrderCount }}</span>
              <p class="text monthText">当月销售量（个）</p>
            </div>
            <div class="stats">
              <span class="num monthNum">{{ (monthOrderAmount/1000000).toFixed(2) }}</span>
              <p class="text monthText">当月销售额（万元）</p>
            </div>
            <div class="stats">
              <span class="num monthNum">{{ (monthTotalBill/100).toFixed(1) }}</span>
              <p class="text monthText">当月分成（元）</p>
            </div>
          </div>
        </div>
      </el-col>
    </el-row>
  </div>
</template>

<script>
import { getOrderCountAPI, getOrderAmountAPI, getTotalBillAPI } from '@/api'
import moment from 'moment'
import dayjs from 'dayjs'
export default {
  data() {
    return {
      monthStartTime: '',
      yearStartTime: '',
      weekStartTime: '',
      dayStartTime: '',
      endTime: '',
      dayOrderCount: '',
      monthOrderCount: '',
      dayOrderAmount: '',
      monthOrderAmount: '',
      dayTotalBill: '',
      monthTotalBill: '',
      loading: false
    }
  },
  created() {
    this.getTime()
    this.getOrderCount()
  },
  methods: {
    async getOrderCount() {
      try {
        this.loading = true
        // 获取订单总数
        const res1 = await getOrderCountAPI(this.dayStartTime, this.endTime)
        this.dayOrderCount = res1.data
        const res2 = await getOrderCountAPI(this.monthStartTime, this.endTime)
        this.monthOrderCount = res2.data
        // 获取收入
        const res3 = await getOrderAmountAPI(this.dayStartTime, this.endTime)
        this.dayOrderAmount = res3.data
        const res4 = await getOrderAmountAPI(this.monthStartTime, this.endTime)
        this.monthOrderAmount = res4.data
        // 获取分成
        const res5 = await getTotalBillAPI(this.dayStartTime, this.endTime)
        this.dayTotalBill = res5.data
        const res6 = await getTotalBillAPI(this.monthStartTime, this.endTime)
        this.monthTotalBill = res6.data
      } finally {
        this.loading = false
      }
    },
    getTime() {
      this.dayStartTime = moment().format('YYYY-MM-DD 00:00:00')
      this.endTime = dayjs(new Date()).format('YYYY-MM-DD HH:mm:ss')
      // 获取当前时间为本周
      const weekOfday = parseInt(moment().format(this.endTime))
      // 本周的开始时间  计算今天是这周第几天 周日为一周中的第一天
      this.weekStartTime = (moment().subtract(weekOfday, this.endTime).format('YYYY-MM-DD 00:00:00'))
      // 获取本月开始时间
      const start = moment().add('month', 0).format('YYYY-MM') + '-01'
      this.monthStartTime = moment(start).format('YYYY-MM-DD HH:mm:ss')
      // 获取本年开始时间
      this.yearStartTime = moment().startOf('year').format('YYYY-MM-DD HH:mm:ss')
      // console.log(this.endTime, this.weekStartTime, this.monthStartTime, this.yearStartTime)
    }
  }
}
</script>

<style lang="scss" scoped>
.bg-purple{
    background: #e9f3ff;
    background-image: url('data:image/png;base64,'),url(http://likede2-admin.itheima.net/img/day.4272ef60.png);
    background-repeat: no-repeat,no-repeat;
    background-position: 0 0,calc(100% - 12px) 100%;
}
.bg-purple-light{
  background: #fbefe8 url(http://likede2-admin.itheima.net/img/month.5c71f45f.png) no-repeat calc(100% - 12px) 100%;
}
.grid-content{
  display: -webkit-box;
  display: -ms-flexbox;
  display: flex;
  padding: 20px;
  border-radius: 20px;
  -webkit-box-orient: vertical;
  -webkit-box-direction: normal;
  -ms-flex-direction: column;
  flex-direction: column;
  height: calc(25vh - 30px);
  min-height: 166px;
  .title{
    font-size: 16px;
    font-weight: 600;
    color: rgb(51, 51, 51);
  }
    .body{
    display: flex;
    flex: 1;
    -webkit-box-flex: 1;
    justify-content: space-around;
    align-items: center;
    font-family: PingFangSC-Semibold,PingFang SC;
    .stats{
      display: inline-flex;
      -webkit-box-orient: vertical;
      -webkit-box-direction: normal;
      flex-direction: column;
    }
    .num{
      height: 50px;
      font-size: 36px;
      font-weight: 600;
      text-shadow: 2px 4px 7px rgb(85 132 255 / 50%);
      color: #072074;
      line-height: 50px;
    }
    .text{
      height: 17px;
      margin-top: 3px;
      font-size: 12px;
      font-weight: 400;
      color: #91a7dc;
      line-height: 17px;
    }
  }
  .monthNum.num {
      color: #ff5757;
      text-shadow: 2px 4px 7px rgb(255 99 85 / 50%);
    }
  .monthText.text {
    color: #de9690;
  }
}
</style>
